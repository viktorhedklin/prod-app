...
import type { DailyEntry, Tier } from '../types';
  ...
  return (
    <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', py: 1, borderBottom: '1px solid', borderColor: 'divider', '&:last-child': { borderBottom: 'none' } }}>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
        <IconButton size="small" onClick={() => handleFlash('down', onDecrement)} disabled={min !== undefined && value <= min} sx={{ border: '1px solid', borderColor: 'divider', borderRadius: 1, width: 28, height: 28 }}>
          <RemoveIcon sx={{ fontSize: 14 }} />
        </IconButton>
        <Fade in={flash !== null} key={flash ?? 'idle'} timeout={150}>
          <Typography sx={{ width: 36, textAlign: 'center', fontWeight: 700, fontSize: '0.9375rem', fontVariantNumeric: 'tabular-nums', color: flash === 'up' ? 'success.main' : flash === 'down' ? 'error.main' : 'text.primary' }}>
            {value}
          </Typography>
        </Fade>
        <IconButton size="small" onClick={() => handleFlash('up', onIncrement)} sx={{ border: '1px solid', borderColor: 'divider', borderRadius: 1, width: 28, height: 28 }}>
...
function CsatRatingInput({ ratings, onAdd, onRemove }: { ratings: number[]; onAdd: (rating: number) => void; onRemove: (index: number) => void }) {
  ...
  return (
    <Box>
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 1 }}>
        ...
        <Typography variant="caption" color="text.secondary">Avg: <strong>{avg}</strong> ({ratings.length} {ratings.length === 1 ? 'response' : 'responses'})</Typography>
      </Box>
      <Box sx={{ display: 'flex', gap: 0.5, mb: 1 }}>
        {[1, 2, 3, 4, 5].map((star) => (
          <IconButton key={star} size="small" onClick={() => onAdd(star)} onMouseEnter={() => setHovered(star)} onMouseLeave={() => setHovered(null)} sx={{ p: 0.5, border: '1px solid', borderColor: 'divider', borderRadius: 1 }}>
            {(hovered !== null ? star <= hovered : false) ? <StarIcon sx={{ fontSize: 18, color: 'warning.main' }} /> : <StarBorderIcon sx={{ fontSize: 18 }} />}
          </IconButton>
        ))}
      </Box>
      <Fade in={ratings.length > 0}>
        <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
          {ratings.map((r, i) => (
            <Chip key={`${r}-${i}`} label={`${r}★`} size="small" onDelete={() => onRemove(i)} deleteIcon={<CloseIcon sx={{ fontSize: 14 }} />} sx={{ bgcolor: 'action.selected', fontSize: '0.75rem', height: 24 }} />
          ))}
        </Box>
      </Fade>
    </Box>
...
function LiveScoreRing({ score }: { score: number | null; grade?: Tier | null }) {
  ...
  return (
    <Box sx={{ position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
      <CircularProgress variant="determinate" value={score === null ? 0 : displayPct} size={74} thickness={5} sx={{ color: 'common.white', opacity: score === null ? 0.35 : 1 }} />
...
export default function Today() {
  ...
  const datesWithData = useMemo(() => new Set(
  ...
  const liveTiers = useMemo(() => targets.map((t) => {
    const val = aggregated[t.metric_key];
    return { label: t.label, metric_key: t.metric_key, tier: tierFromValue(typeof val === 'number' ? val : null, t.thresholds, t.direction) };
  }), [aggregated, targets]);
  ...
  return (
    <Box sx={{ p: { xs: 2, md: 3 }, maxWidth: 900, mx: 'auto' }}>
      ...
      <Box sx={{ mb: 2, p: 2, display: 'flex', alignItems: 'center', gap: 2, borderRadius: 3, bgcolor: 'primary.main', color: 'primary.contrastText' }}>
        ...
        <Grid size={{ xs: 12, md: 6 }}>
          <StatCard title="Volume" interactive delay={50}>
            {STEPPER_FIELDS.map((f) => (
              <StepperRow key={f.key} label={f.label} hint={f.hint} value={Number(entry[f.key] ?? 0)} onIncrement={() => handleStep(f.key, 1, f.min)} onDecrement={() => handleStep(f.key, -1, f.min)} min={f.min} />
            ))}
          </StatCard>
          <Box sx={{ mt: 2 }}>
        ...
        <Grid size={{ xs: 12, md: 6 }}>
          <StatCard title="Quality & Escalation" interactive delay={100}>
            ...
            <Box sx={{ mt: 1.5, p: 1.5, borderRadius: 2, bgcolor: 'action.hover' }}>
              <Typography variant="caption" color="text.secondary">Submitted task hours</Typography>
              <Typography sx={{ fontWeight: 700, fontSize: '1.25rem' }}>{Number(entry.task_hours_submitted || 0).toFixed(1)}h</Typography>
              <Typography variant="caption" color="text.secondary">Filled only when you mark a Shift Todo submitted. This field is locked so pending hours cannot leak into the score.</Typography>
            </Box>
          </StatCard>
          <Box sx={{ mt: 2 }}>
            <StatCard title="CSAT Ratings" interactive delay={150}>
              <CsatRatingInput ratings={entry.csat_ratings} onAdd={(rating) => { updateEntry(date, { csat_ratings: [...entry.csat_ratings, rating] }); notify(`${rating}-star CSAT rating added`); }} onRemove={(index) => updateEntry(date, { csat_ratings: entry.csat_ratings.filter((_, i) => i !== index) })} />
            </StatCard>
          </Box>
          <Box sx={{ mt: 2 }}>
            <StatCard title="Shift Todo Progress" delay={180}>
              <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.75 }}>
                <Typography variant="body2" sx={{ fontWeight: 600 }}>{dayTodos.length === 0 ? 'No todos for this day' : `${dayTodos.length - pendingTasks.length}/${dayTodos.length} submitted`}</Typography>
                <Typography variant="caption" color={todoProgress === 100 ? 'success.main' : 'warning.main'}>{todoProgress}%</Typography>
              </Box>
              ...
              <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1 }}>
                {liveTiers.map((lt) => (
                  <Box key={lt.metric_key} sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                    <Typography variant="caption" color="text.secondary" sx={{ fontWeight: 500 }}>{lt.label}:</Typography>
...
function PointRow({ label, value }: { label: string; value: number }) {
  return (
    <Box sx={{ display: 'flex', justifyContent: 'space-between', py: 0.4 }}>
      ...
      <Typography variant="body2" sx={{ fontWeight: 600, fontVariantNumeric: 'tabular-nums' }}>{value.toFixed(1)}</Typography>
    </Box>
  );
}
